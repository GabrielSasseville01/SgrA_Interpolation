...
class SimulatedDataset(Dataset):
    def __init__(self, data):
        ...
        ## REMOVE UP TO HERE
        # Normalize ground truth between 0 and 1
        min_val = self.ground_truth.min()
        max_val = self.ground_truth.max()
        self.ground_truth = (self.ground_truth - min_val) / (max_val - min_val + 1e-8)  # Adding small epsilon to avoid division by zero
        ## REMOVE UP TO HERE
...
def get_dataloader(seed=1, nfold=None, batch_size=16, missing_ratio=0.1):
    ...
    # Load the .npz file
    file_path = "data/random_walk_1d.npz"  # Update with your file path
