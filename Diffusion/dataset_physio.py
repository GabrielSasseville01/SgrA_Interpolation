...
def parse_id(id_, missing_ratio=0.1):
    data = pd.read_csv(DATA_DIR / "physio/set-a/{}.txt".format(id_))
    # set hour
    extract_relative_minutes = lambda x: (int(x[0]) * 60 + int(x[1])) / 60 / 48
    data['relativeTime'] = data['Time'].str.split(':').apply(extract_relative_minutes)
    ...
    for h in range(48):
        ...
        times = data[data['Time'] == h]['relativeTime']
...
def get_idlist():
    patient_id = []
    for filename in os.listdir(DATA_DIR / "physio/set-a"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.observed_times = []
        path = (
            DATA_DIR / ("physio_missing" + str(missing_ratio) + "_seed" + str(seed) + ".pk")
        )
        ...
        if os.path.isfile(path) == False:  # if datasetfile is none, create
            ...
            for id_ in idlist:
                try:
                    observed_values, observed_masks, gt_masks, observed_times = parse_id(
                        id_, missing_ratio
                    )
            ...
            self.observed_times = np.array(self.observed_times)
            ...
            with open(path, "wb") as f:
                pickle.dump(
                    [self.observed_values, self.observed_masks, self.gt_masks, self.observed_times], f
                )
        else:  # load datasetfile
            with open(path, "rb") as f:
                self.observed_values, self.observed_masks, self.gt_masks, self.observed_times = pickle.load(
                    f
                )
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            ...
            "times": self.observed_times[index],
        }
        return s
